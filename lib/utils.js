...
utils.matchContext = function(context, url) {
    ...
    positiveContexts = contexts.filter(function(c){return c.charAt(0) !== '!';});
    negativeContexts = contexts.filter(function(c){return c.charAt(0) === '!';});
    // Remove the '!' character from the contexts
    negativeContexts = negativeContexts.map(function(c){return c.slice(1);});
    negativeMatch = negativeContexts.find(function(c){return url.lastIndexOf(c, 0) === 0;});
    // If any context negates this url, it must not be proxied.
    if (negativeMatch) {
        return false;
    }
    positiveMatch = positiveContexts.find(function(c){return url.lastIndexOf(c, 0) === 0;});
    ...
    proxies.forEach(function(proxy) {
        if (!proxied && req && utils.matchContext(proxy.config.context, req.url)) {
            ...
            if (proxy.config.headers != null) {
                Object.entries(proxy.config.headers).forEach(function(value, key) {
